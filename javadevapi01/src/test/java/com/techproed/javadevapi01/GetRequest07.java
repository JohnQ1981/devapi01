...
public class GetRequest07 extends TestBaseHerOkuApp {
	...
	public void get01() {
		...
		//Send the Request
		Response response = given().spec(spec).accept("application/json").when().get("/{bookingName}/{idValue}");
		...
		//2nd Way JsonPath Class+ Hard Assertion
		//JsonPath Class is good to navigate and to assert for JSON Data

		//Navigate and Assert the JSON Data

		JsonPath json = response.jsonPath();

		//		assertEquals(200,response.getStatusCode());
		//		assertEquals("Mark", json.getString("firstname"));
		//		assertEquals("Ericsson", json.get("lastname"));// generic get () but not recommended
		...
		//3RD way USE JsonPath Class + SoftAssertion
		//Create Soft 
